...
\newcommand*{\assignment}{有限展示与有限生成模}
...
\begin{proposition}
    给定范畴 $\mathcal C$ 上的系统 $M_{()}:I\to \mathcal C$, 其中, $M_{()}:i\mapsto M_i$, $M_\alpha\in \mathrm{Hom}_{\mathcal C}(M_{s(\alpha)},M_{t(\alpha)})$. 若相应的极限存在, 则有自然同构
    \begin{itemize}
        \item $\mathrm{Hom}_{R}(N,\varprojlim_{i\in I}M_i)\simeq \varprojlim_{i\in I}\mathrm{Hom}_{R}(N,M_i)$,
        \item $\mathrm{Hom}_{R}(\varinjlim_{i\in I}M_i,N)\simeq \varprojlim_{i\in I}\mathrm{Hom}_{R}(M_i,N)$.
    \end{itemize}
    \begin{proof}
        下仅证明第一条, 往后证明步骤舍去 $\mathrm{Hom}_{\mathcal C}$.
    \end{proof}
\end{proposition}
